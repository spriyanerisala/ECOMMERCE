    ...
    return (
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
            ...
            <NavLink  to="/" onClick={()=>setOpen(false)}>
                <img src={assets.veg_shop_logo} className='h-15'  alt="" />
            </NavLink>
            ...
            {(
               !user ? ( <button onClick={()=>setShowUserLogin(true)} className="cursor-pointer px-8 py-2 bg-green-500  hover:bg-green-400 transition text-white rounded-full">
                    Login
                </button> )
                :
                (
             ...
             open && ( <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
               ...
               <NavLink to='/contact' onClick={()=>setOpen(false)}>Contact</NavLink>
               {
                !user ? (<button   onClick={()=>{
                    ...
                    <button
                    onClick={logout} className="cursor-pointer px-6 py-2 mt-2 bg-green-500 hover:bg-green-400-dull transition text-white rounded-full text-sm">
                    Logout
                </button>
                )  
               }
